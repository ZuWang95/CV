...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        ...
        step_dx, dprev_h, step_dWx, step_dWh, step_db = rnn_step_backward(dcurrent_h, cache[t])
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        pooled_features = image_features.mean(dim=(2, 3))
        h0 = self.feature_projection(pooled_features)  # (N, H)

        embedded_words = self.word_embedding(captions_in)  # (N, T, W)
        if self.cell_type == "rnn":
          hiddens = self.rnn(embedded_words, h0)  # (N, T, H)
        elif self.cell_type == "lstm":
          hiddens = self.lstm(embedded_words, h0)
        else:
          A = self.attn_proj(image_features)
          hiddens = self.attn_lstm(embedded_words, A)

        scores = self.output_projection(hiddens)
    ...
    def sample(self, images, max_length=15):
      ...
      Args:
          images: Input images of shape (N, 3, H, W)
          max_length: Maximum number of words to generate

      Returns:
          captions: Tensor of shape (N, max_length) with word indices
          attn_weights_all (optional): Tensor of shape (N, max_length, H_feat, W_feat)
                                      if using attention, otherwise not returned
      """
      N = images.shape[0]
      captions = self._null * images.new(N, max_length).fill_(1).long()

      if self.cell_type == "attn":
          A = self.attn_proj(self.image_encoder(images))  # (N, H, H_feat, W_feat)
          h = c = A.mean(dim=(2, 3))  # initialize hidden and cell state from mean-pooled A
          attn_weights_all = images.new(N, max_length, A.shape[2], A.shape[3]).fill_(0).float()
      else:
          img_feats = self.image_encoder(images)  # (N, C, H_feat, W_feat)
          img_feats = img_feats.mean(dim=(2, 3))  # (N, C)
          h = self.feature_projection(img_feats)  # (N, H)
          c = torch.zeros_like(h)

      current_word = torch.full((N,), self._start, dtype=torch.long, device=images.device)

      for t in range(max_length):
          word_embed = self.word_embedding(current_word)  # (N, W)
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        # Replace "pass" statement with your code
        a = x @ self.Wx + prev_h @ self.Wh + self.b  # shape (N, 4H)
        ...
        g = torch.tanh(ag)            # cell candidate

        next_c = f * prev_c + i * g   # update cell state
        next_h = o * torch.tanh(next_c)  # update hidden state
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T):
          ...
          hn_list.append(next_h.unsqueeze(1))  # (N, 1, H)
...
def dot_product_attention(prev_h, A):
    ...
    attn_logits = torch.bmm(prev_h.unsqueeze(1), A_flat)  # (N, 1, 16)
    attn_logits = attn_logits.squeeze(1) / scale

    alpha = torch.softmax(attn_logits, dim=-1)

    attn = torch.bmm(A_flat, alpha.unsqueeze(2)).squeeze(2)

    attn_weights = alpha.view(N, 4, 4)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(T):
            x_t = x[:, t, :]  # (N, D)
            attn, _ = dot_product_attention(h, A)  # (N, H), (N, 4, 4)
            h, c = self.step_forward(x_t, h, c, attn)
            h_list.append(h.unsqueeze(1))  # (N, 1, H)

        hn = torch.cat(h_list, dim=1)  # (N, T, H)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        return hn
