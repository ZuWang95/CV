...
def svm_loss_vectorized(
    ...
    margins = torch.clamp(scores - correct_class_scores + 1, min=0)  # shape: (N, C)
...
def test_one_param_set(
    ...
    train_acc = (
        (y_train_pred == data_dict["y_train"]).float().mean().item() * 100.0
    )

    y_val_pred = predict_linear_classifier(cls.W, data_dict["X_val"])
    val_acc = (
        (y_val_pred == data_dict["y_val"]).float().mean().item() * 100.0
    )
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
