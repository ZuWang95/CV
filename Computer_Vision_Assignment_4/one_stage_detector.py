...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for ch in stem_channels:
            conv = nn.Conv2d(prev_channels, ch, kernel_size=3, stride=1, padding=1)
        ...
        last_channels = stem_channels[-1]

        self.pred_cls = nn.Conv2d(last_channels, num_classes, kernel_size=3, stride=1, padding=1)
        self.pred_box = nn.Conv2d(last_channels, 4, kernel_size=3, stride=1, padding=1)
        self.pred_ctr = nn.Conv2d(last_channels, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, feat in feats_per_fpn_level.items():
            feat_cls = self.stem_cls(feat)
            feat_box = self.stem_box(feat)

            cls_out = self.pred_cls(feat_cls)
            box_out = self.pred_box(feat_box)
            ctr_out = self.pred_ctr(feat_box)

            # reshape from (B, C, H, W) to (B, H*W, C)
            B, C, H, W = cls_out.shape
            class_logits[level_name] = cls_out.permute(0, 2, 3, 1).reshape(B, H * W, C)

            B, C, H, W = box_out.shape
            boxreg_deltas[level_name] = box_out.permute(0, 2, 3, 1).reshape(B, H * W, C)

            B, C, H, W = ctr_out.shape
            centerness_logits[level_name] = ctr_out.permute(0, 2, 3, 1).reshape(B, H * W, C)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.full((locations.shape[0], 4), -1.0, device=locations.device)
    foreground_mask = (gt_boxes[:, 0] >= 0)

    deltas[foreground_mask, 0] = (xc[foreground_mask] - x1[foreground_mask]) / stride  
    deltas[foreground_mask, 1] = (yc[foreground_mask] - y1[foreground_mask]) / stride  
    deltas[foreground_mask, 2] = (x2[foreground_mask] - xc[foreground_mask]) / stride  
    deltas[foreground_mask, 3] = (y2[foreground_mask] - yc[foreground_mask]) / stride
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    left_right_min = torch.minimum(l[foreground_mask], r[foreground_mask])
    left_right_max = torch.maximum(l[foreground_mask], r[foreground_mask])
    top_bottom_min = torch.minimum(t[foreground_mask], b[foreground_mask])
    top_bottom_max = torch.maximum(t[foreground_mask], b[foreground_mask])

    eps = 1e-6  
    centerness_val = torch.sqrt(
        (left_right_min / (left_right_max + eps)) *
        (top_bottom_min / (top_bottom_max + eps))
    )

    centerness[foreground_mask] = centerness_val
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = \
          self.pred_net(feats_per_fpn_level) 

        shape_per_fpn_level = {
            k: v.shape for k, v in feats_per_fpn_level.items()
        }
        ######################################################################
        ...
        B = images.shape[0]
        matched_gt_boxes = []          # list length = B
        matched_gt_deltas = []

        for i in range(B):
            per_img_gt = gt_boxes[i]                 # (Mi,5)
            ...
            per_img_deltas = {
                lvl: fcos_get_deltas_from_locations(
                    locations_per_fpn_level[lvl],
                    matched_boxes[lvl],
                    strides_per_fpn_level[lvl],
                )
                for lvl in matched_boxes.keys()
            }
            matched_gt_deltas.append(per_img_deltas)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        cls_targets = pred_cls_logits.new_zeros(B, N, C)

        fg_mask = matched_gt_boxes[..., 4] >= 0
        cls_inds = matched_gt_boxes[..., 4].clamp_min(0).long()
        cls_targets[fg_mask, cls_inds[fg_mask]] = 1.0

        pred_cls_logits = pred_cls_logits.clamp_(-10, 10)
        loss_cls = sigmoid_focal_loss(
            pred_cls_logits, cls_targets,
            reduction="none", alpha=0.25, gamma=2.0
        ).sum(dim=-1)

        loss_box = pred_cls_logits.new_zeros(B, N)
        if fg_mask.any():                              
            l1 = F.l1_loss(
                pred_boxreg_deltas[fg_mask],            
                matched_gt_deltas[fg_mask],             
                reduction="none",
            ).sum(dim=-1)                              
            loss_box[fg_mask] = l1 

        loss_ctr = pred_cls_logits.new_zeros(B, N)        
        ctr_targets = fcos_make_centerness_targets(       
            matched_gt_deltas.view(-1, 4)
        ).view(B, N)
        ctr_mask = ctr_targets >= 0                     
        if ctr_mask.any():
            loss_ctr[ctr_mask] = F.binary_cross_entropy_with_logits(
                pred_ctr_logits.squeeze(-1)[ctr_mask],    
                ctr_targets[ctr_mask],                  
                reduction="none",
            )
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            stride = self.backbone.fpn_strides[level_name]   # 8 / 16 / 32
            img_h, img_w = images.shape[2], images.shape[3]  # H, W of input
                             # (H·W, num_cls)
            ...
            N, C = level_pred_scores.shape 
            # Step 1:
            # Replace "pass" statement with your code
            level_best_scores, level_best_classes = level_pred_scores.max(dim=1)

            # Step 2:
            # Replace "pass" statement with your code
            keep_mask = level_best_scores > test_score_thresh
            if keep_mask.sum() == 0:                        # nothing left
                continue

            level_best_scores   = level_best_scores[keep_mask]
            level_best_classes  = level_best_classes[keep_mask]
            locs_kept           = level_locations[keep_mask]
            deltas_kept         = level_deltas[keep_mask]   # (M,4)

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(
                deltas_kept, locs_kept, stride
            )   

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            level_pred_boxes[:, 0::2] = level_pred_boxes[:, 0::2].clamp(0, img_w)
            level_pred_boxes[:, 1::2] = level_pred_boxes[:, 1::2].clamp(0, img_h)

            # -------------------------------------------------------------
            # collect results from this FPN level
            level_pred_scores  = level_best_scores          # rename for clarity
            level_pred_classes = level_best_classes
            ##################################################################
            #                          END OF YOUR CODE                      #
            ##################################################################
        ...
        ######################################################################
        if len(pred_boxes_all_levels) == 0:           # nothing survived on any level
          device = images.device
          ...
          return empty_boxes, empty_classes, empty_scores
